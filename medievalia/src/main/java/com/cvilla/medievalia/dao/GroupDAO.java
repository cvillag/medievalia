...
public class GroupDAO implements IGroupDAO {
	...
	private static final String GET_USER_NOT_IN_GROUP = "select user_id, user_name, user_long_name, user_role, ' ' as user_pass from users where user_id not in (select idStudent from students where idGroup = ?) and user_id not in (select idTeacher from teachers where idGroup = ?) and user_id != 0  and (user_role = 2 or user_role = 3)";
	...
	public List<User> getPossibleUsersListToGroup(int idGroup, String filter){
		...
		try{
			if(filter == null || filter.length() < 1){
				list = jdbcTemplate.query(GET_USER_NOT_IN_GROUP, new Object[]{idGroup,idGroup},new UserMapper());
			}
			else{
				list = jdbcTemplate.query(GET_USER_NOT_IN_GROUP_FILTER, new Object[]{idGroup,idGroup,filter},new UserMapper());
			}
			return list;
		}
		catch(Exception e){
