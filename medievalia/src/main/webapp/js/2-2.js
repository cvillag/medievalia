...
function cargaAtributosComplejosPorPagina(pag2,recarga){
	...
	function(data){
		...
		for(var d in obj){
			...
			var cpagina = obj[d].conPaginaDoc;
			var val = obj[d].validado;
			if(rol == 3){
				if(val == 0){
					if(idUser == obj[d].creador){
						if(obj[d].textoLeido == 1){
							$("#list"+pagina).append('<li class="list-group-item liac1-'+pagina+'" data-name="'+nom+'" id="'+tipo+'-'+idInst+'"><button type="button" id="remAtC'+tipo+'-'+idInst+'" class="btn btn-xs btn-default remComplexAttribute" data-tipo="'+pagina+'" data-inst="'+idInst+'" data-pag="'+pagina+'" data-name="'+nom+'" data-tiporelacion="'+relacion+'" data-cfecha="'+ cfecha +'" data-cpagina="'+cpagina+'" title="Eliminar atributo"><span class="glyphicon glyphicon-arrow-right"></span></button><button type="button" class="btn btn-xs btn-default modifyRelation" data-tipo="'+pagina+'" data-inst="'+idInst+'" data-instp="' + idInstP + '"  title="Modificar información del atributo"><span class="glyphicon glyphicon-pencil"></span></button> '+nom+'</li>');
						}
						else{
						...
						$("#list"+pagina).append('<li class="list-group-item liac1-'+pagina+'" data-name="'+nom+'" id="'+tipo+'-'+idInst+'"><button type="button" class="btn btn-xs btn-warning pendienteValOtro" title="Atributo pendiente de validar"><span class="glyphicon glyphicon-ban-circle"></span></button><button type="button" class="btn btn-xs btn-default viewRelation" data-tipo="'+pagina+'" data-inst="'+idInst+'" data-instp="' + idInstP + '" title="Ver información del atributo"><span class="glyphicon glyphicon-eye-open"></span></button> '+nom+'</li>');
					}
				}
				else{
					$("#list"+pagina).append('<li class="list-group-item liac1-'+pagina+'" data-name="'+nom+'" id="'+tipo+'-'+idInst+'"><button type="button" class="btn btn-xs btn-success yavalidado" title="Atributo ya validado"><span class="glyphicon glyphicon-ban-circle"></span></button><button type="button" class="btn btn-xs btn-default viewRelation" data-tipo="'+pagina+'" data-inst="'+idInst+'" data-instp="' + idInstP + '" title="Ver información del atributo"><span class="glyphicon glyphicon-eye-open"></span></button> '+nom+'</li>');
				}
			}
			else{
				if(val == 1){
					...
					$("#list"+pagina).append('<li class="list-group-item liac1-'+pagina+'" data-name="'+nom+'" id="'+tipo+'-'+idInst+'"><button type="button" class="btn btn-xs btn-warning pendienteVal" title="Atributo pendiente de validar"><span class="glyphicon glyphicon-ban-circle"></span></button><button type="button" class="btn btn-xs btn-default viewRelation" data-tipo="'+pagina+'" data-inst="'+idInst+'" data-instp="' + idInstP + '" title="Ver información del atributo"><span class="glyphicon glyphicon-eye-open"></span></button> '+nom+'</li>');
				}
			}
		}
		...
		for(var d in disp){
			...
			$("#listD"+pagina).append('<li class="list-group-item liac2-'+pagina+'" data-name="'+nom+'" id="'+tipo+'-'+idInst+'"><button type="button" id="addAtC'+tipo+'-'+idInst+'" class="btn btn-xs btn-default addComplexAttribute" data-tipo="'+pagina+'" data-inst="'+idInst+'" data-pag="'+pagina+'" data-name="'+nom+'" data-tiporelacion="'+relacion+'" data-cfecha="'+ cfecha +'" data-cpagina="'+cpagina+'" title="Añadir atributo">&nbsp;<span class="glyphicon glyphicon-arrow-left"></span></button> '+nom+'</li>');
		}
...
var handViewRel = function botonViewVal(){
	...
	function(data){
		...
		if(json.message = "ok"){
			...
			if(json.conPaginaDoc == 1 && json.paginaDoc != 0){
				$("#paginaDocV" + tipo).val(json.paginaDoc);
			}
			else{
				$("#paginaDocM" + tipo).val("");
			}
...
var handModifyRel = function botonModifyVal(){
	...
	function(data){
		...
		if(json.message = "ok"){
			...
			if(json.conPaginaDoc == 1 && json.paginaDoc != 0){
				$("#paginaDocM" + tipo).val(json.paginaDoc);
			}
			else{
				$("#paginaDocM" + tipo).val("");
			}
			$("#conFechaM" + tipo).val(json.conFecha);
			$("#conPaginaDoc" + tipo).val(json.conPaginaDoc);
...
//	else
	...
	$(".buttonmodalasignacionrelacion").click(function(){
		...
		conPaginaDoc = $("#conPaginaDoc" + tipoR).val();
		paginaDoc = $("#paginaDoc" + tipoR).val();
		diai = $("#diaI" + tipoR).val();
		mesi = $("#mesI" + tipoR).val();
	...
	$(".buttonmodalmodifyrelacion").click(function(){
		...
		conPaginaDoc = $("#conPaginaDocM" + tiporm).val();
		paginaDoc = $("#paginaDocM" + tiporm).val();
