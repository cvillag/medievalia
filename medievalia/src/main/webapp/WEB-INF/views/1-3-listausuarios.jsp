...
<div class="container">
	<legend>
		<fmt:message key="p1-3.bienvenida" />
	</legend>
		...
		<%
			...
			<div class="row">
				...
				<%if(u.getUser_role() == 1){ %>
						...
						<%} %>
						<div class="col-xs-3">
							<button type="button" class="btn btn-info navbar-btn deleteuser" data-val="<%=u.getId() %>">
								<span class="glyphicon glyphicon-remove" aria-hidden="true"></span>
							</button>
							<button type="button" class="btn btn-info navbar-btn modifyuser" data-val="<%=u.getId() %>">
								<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>
							</button>
							<button type="button" class="btn btn-info navbar-btn detailsuser" data-val="<%=u.getId() %>">
			...
			<table
				...
				for(User u : list){ %>
					<tr id="fila<%=u.getId()%>">
						...
						<%if(u.getUser_role() == 1){ %>
						<td width="5%"><span class="glyphicon glyphicon-cog"
							title="<fmt:message key="general.administrador"/>"></span></td>
						<%} 
						else if(u.getUser_role() == 2){%>
						<td width="5%"><span class="glyphicon glyphicon-briefcase"
							title="<fmt:message key="general.profesor"/>"></span></td>
						<%} 
						else if(u.getUser_role() == 3){%>
						<td width="5%"><span class="glyphicon glyphicon-user"
							title="<fmt:message key="general.alumno"/>"></span></td>
						<%}
						else if(u.getUser_role() == 4){%>
						<td width="5%"><span class="glyphicon glyphicon-ban-circle"
		...
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">
					<fmt:message key="p1-3.modalborrar1"></fmt:message>
				</h4>
			</div>
			<div class="modal-body">
				<p>
					<fmt:message key="p1-3.modalborrar2"></fmt:message>
				</p>
			</div>
			<div class="modal-footer">
				<button type="button" id="modalok" class="btn btn-sm btn-info"
					data-dismiss="modal">
					<fmt:message key="p1-3.modalborrar"></fmt:message>
				</button>
				<button type="button" id="modalcancel" class="btn btn-sm btn-info"
					data-dismiss="modal">
					<fmt:message key="p1-3.modalcancelar"></fmt:message>
				</button>
			</div>
		</div>
