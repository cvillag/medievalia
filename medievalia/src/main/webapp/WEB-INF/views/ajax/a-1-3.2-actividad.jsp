...
<table
	...
	<tbody>
		...
		<tr>
			...
			<td>
				<% if(l.getSuccess()==1){%><fmt:message key="p1-3.2.t2-exito" />
				<%}else{ %><fmt:message key="p1-3.2.t2-noexito" />
				<%} %>
			</td>
		</tr>
		<%}
...
<div class="alert alert-danger">
	<fmt:message key="p1-3.2.error.paginaNoExiste" />
</div>
<%} %>
<%} %>
<form action="activityUserA.do" id="pagForm" method="post">
	<input type="hidden" name="pag" id="pag" value="1"> <input
		type="hidden" name="tamPag" id="tamPag" value="10"> <input
		type="hidden" name="detailId" id="detailId"
		value="<%=request.getAttribute("detailId")%>"> <input
		type="hidden" name="maxPag" id="maxPag" value="<%=numPags%>">
